...
df_mcf_bc = df_mcf['BC'].unique().tolist()

def alphanumeric_key(s):
    # Split string into parts: ['R', 10, '/', 11] etc.
    return [
        int(text) if text.isdigit() else text.lower()
        for text in re.split(r'(\d+)', s)
    ]
df_mcf_use = sorted(df_mcf['Use'].unique().tolist(), key=alphanumeric_key)+["All"]


def adjust_bar_width(fig, df, x_col):
    """Auto-adjust bar width and spacing based on number of x categories."""
    n_bars = len(df[x_col].unique())

    if n_bars == 1:
        # 🟩 Single bar: narrow and centered
        fig.update_xaxes(range=[-0.5, 0.5])
        fig.update_traces(selector=dict(type="bar"), width=0.2)
    elif n_bars == 2:
        # 🟦 Two bars: small gap and medium width
        fig.update_xaxes(range=[-0.5, 1.5])
        fig.update_traces(selector=dict(type="bar"), width=0.3)
    else:
        # 🟨 Normal view
        fig.update_traces(selector=dict(type="bar"), width=0.4)
...
main_app=html.Div([
    ...
    html.Div([
        html.Div([
            html.Label("Select Use:", style={"font-weight": "bold", "margin-right": "10px"}),
            dcc.Dropdown(
                id="select-use",
                options=[{"label": u, "value": u} for u in df_mcf_use],
                multi=False,
                value=df_mcf_use[0],
                style={"width": "200px"}
            ),
        ], style={"display": "flex", "align-items": "center", "gap": "10px", "margin-right": "40px"}),

        html.Div([
            html.Label("Display:", style={"font-weight": "bold", "margin-right": "10px"}),
            dcc.RadioItems(
                id="mcf-toggle",
                options=[
    ...
    dcc.Graph(id="Graph9-LossRatio_MCF", style={ 'margin-bottom': '20px'}),
    ...
    html.Div([
        ...
        html.Div([
            html.Label("Select Use:"),
            dcc.Dropdown(
                id="select-use2",
                options=[{"label": u, "value": u} for u in df_mcf_use],
                multi=False,
                value=df_mcf_use[0]  # default all
            ),
...
def register_main_callbacks(app):
    ...
    def premium_bar_chart2(selected_year,selected_class,selected_cutoff,selected_info):
        ...
        if selected_info == "Total Premium":
            ...
            fig = adjust_bar_width(fig, summary, "Cls")
    ...
    @app.callback(
        ...
        Input("select-use", "value")]
    )
    # MCF loss ratio
    def loss_ratio_mcf(toggle_value, selected_year,selected_use):
        if isinstance(selected_use, str):
            selected_use = [selected_use]

        if selected_use == ["All"]:
            filtered_df = df_mcf[
                (df_mcf["UwYr"] == selected_year)
            ].copy()
        else:
            filtered_df = df_mcf[
                (df_mcf["UwYr"] == selected_year) &
                (df_mcf["Use"].isin(selected_use))
        ].copy()
        ...
        filtered_df = (
            filtered_df.groupby(["InsdName","BC"])
            .agg({
        ...
        if toggle_value == "vehicles":
            if pd.notna(latest_cutoff):
                cutoff_label = latest_cutoff.strftime('%b %Y')   # e.g. "Oct 2025"
            else:
                cutoff_label = "No Data"
            fig = px.bar(
                filtered_df,
                x="InsdName",
                y="Vehicles_in_Fleet",   
                color="BC", 
                title=f"MCF Vehicles Count - {cutoff_label}",
            ...
            fig = adjust_bar_width(fig, filtered_df, "InsdName")
            ...
            filtered_df = filtered_df.sort_values(
                by=["BC", "InsdName"],
                ascending=[False, True]  # False puts "Yes" before "No"
            )
            insured_order = filtered_df["InsdName"].unique().tolist()

            loss_ratio_summary = (
                filtered_df.groupby("InsdName", as_index=False)
                .agg({"Loss_Ratio": "mean"})
            )

            loss_ratio_summary = loss_ratio_summary.set_index("InsdName").loc[insured_order].reset_index()
            ...
            fig = px.bar(
                ...
                y="PremiumAmount_cumsum",
                color="BC",
                text="PremiumAmount_cumsum",
                title=f"Premium vs Loss Ratio by Insured and BC - {cutoff_label}",
                barmode="group",
                category_orders={"InsdName": insured_order},
            )
            ...
            # --- Add line trace for Loss Ratio (one per Insured) ---
            fig.add_trace(
                go.Scatter(
                    x=loss_ratio_summary["InsdName"],
                    y=loss_ratio_summary["Loss_Ratio"],
                    mode="lines+markers+text",
                    name="Loss Ratio",
                    yaxis="y2",
                    text=loss_ratio_summary["Loss_Ratio"].apply(lambda x: f"{x:.1%}"),
                    textposition="top center",
    ...
    @app.callback(
        ...
        Input("select-use2", "value"),
    )

    def MCF_burning_cost(selected_year,selected_use):
        if isinstance(selected_use, str):
            selected_use = [selected_use]
        ...
        filtered_df2 = (
            filtered_df
            .groupby(["Tran_MonthYear","Tran_MonthYear_dt","BC"])[["BI_Amount", "PD_Amount", "OD_Amount","WS_Amount","Vehicles_in_Fleet_TP","Vehicles_in_Fleet_OD"]]
            .sum()
            .reset_index()
        )
        ...
        filtered_df2 = filtered_df2.melt(
            id_vars=["Tran_MonthYear", "Tran_MonthYear_dt", "BC"],
            value_vars=["TP", "OD"],
            var_name="Type",          # this will create a column with 'TP' and 'OD'
            value_name="Burning_Cost" # this will hold their numeric values
        )
        ...
        fig.update_layout(
            ...
            legend_title="Loss Type, Burning Cost?",
